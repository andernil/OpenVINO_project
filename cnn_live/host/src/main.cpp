...
scoped_array<cl_mem> input_buf; // or data_buf?
...
// Open input file
const char* input_file_name = "../input/voice_rec_live"; //test_input_1"; //voice_rec_live";
//std::ifstream input_file("input/rec_test", std::ios::in | std::ios::binary);

// Open time log file
std::ofstream log_file("logs/time_log_cu4.log");
...
void prepare(){
	...
	//cnpy::NpyArray input_npy = cnpy::npy_load("../fpga/per_layer/unit_tests/input_1_data.npy"); // not necessary for cnn_live
	cnpy::NpyArray golden_ref_npy = cnpy::npy_load("../fpga/per_layer/unit_tests/input_1_ip3.npy"); //input_1_prob.npy"); //output_1.npy");
...
bool init_opencl() {
  ...
  std::string binary_file = getBoardBinaryFile("cnn_ndrconv_cu4", device[0]); // "cnn_ndrconv_cu4"
  printf("Using AOCX: %s\n", binary_file.c_str());
...
void run() {
  ...
  // Hidden layer 1 kernel
	while(1) {
		tot_kernel_time = 0; // reset tot kernel time
		read_new_input();
		for (int i = 0; i < num_layers; i++) {
			if (layer_config[i][layer_type] == 0) { // conv layer
				int argi;
			...
			if (i == 0) { // first layer
				status = clEnqueueWriteBuffer(queue_conv[0], input_buf[0], CL_FALSE,
			...
			if (layer_config[i][layer_type] == 0) { // conv layer
				...
				status = clEnqueueWriteBuffer(queue_fc[0], weights_buf[i], CL_FALSE,
			  	0, weight_sizes[i] * sizeof(float), weights[i], 0, NULL, NULL);
				checkError(status, "Failed to transfer weights");

				status = clEnqueueWriteBuffer(queue_fc[0], bias_buf[i], CL_FALSE,
			  	0, bias_sizes[i] * sizeof(float), bias[i], 0, NULL, NULL);
				checkError(status, "Failed to transfer bias");

				if (clFinish(queue_fc[0]) == CL_SUCCESS)
					printf("cl_finish == success\n");
...
void verify() { // need to account for some rounding differences
  printf("Verifying\n");
