...
def overall_accuracy_check(model: str, ref_model: str, out_layers: list, ref_out_layers: list, inputs: dict,
                           ref_inputs: dict, plugin: IEPlugin, ref_plugin: IEPlugin, layers: str, num_of_iterations: int):
...
def one_ir_mode(args):
    ...
    print_all_over_the_net_metrics(global_times=global_times, ref_global_times=ref_global_times, global_accuracy=global_accuracy)
...
def two_ir_mode(args):
    ...
    inputs = input_processing(model_path=args.model, net_inputs=net_inputs, input_file=args.input, layers_map=layers_map)
    ref_inputs = input_processing(model_path=args.reference_model, net_inputs=ref_net_inputs, input_file=args.input, layers_map=layers_map)
    global_accuracy = []
