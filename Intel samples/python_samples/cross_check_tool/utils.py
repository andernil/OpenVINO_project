    ...
    def decorator(func):
        def try_except_func(*args, **kwargs):
            try:
                ...
                log.error(
                    "The following error happened while {}:\n{}: {}".format(desc.format(**kwargs), exception_type, e))
                global verbosity
                if verbosity:
...
def validate_args(args):
    ...
    if args.model is None and args.reference_model is None:
        raise Exception("Parameters --model/-m and --reference_model/-ref_m are empty. At least one of them is required")
    ...
    if args.model != args.reference_model and args.reference_model is not None and args.mapping is None and \
        ...
        log.warning('Check over two different IRs was enabled. In case if layer names in this two IRs deffer, '
    ...
    for net_input in net_inputs:
        ...
        if 'blob' in npz[net_input].item(0):
            ...
            log.info('Layer {} shape = {}, input blob from multi-input file shape = {}'
                     ''.format(net_input, net_inputs[net_input], just_blob.shape))
            try:
                reshaped_blob = np.reshape(just_blob, net_inputs[net_input])
            except:
                raise Exception('Can not reshape input blob from multi-input file for layer {} to shape {}'
                                ''.format(net_input, net_inputs[net_input]))
...
def read_image_file(input_file: str, net_inputs: dict):
    # TODO batch and mtreskin samples
    inputs = dict()
    if len(net_inputs) == 1:
        image = cv2.imread(input_file)
        only_layer_name = list(net_inputs.keys())[0]
        shape = net_inputs[only_layer_name]
        if len(shape) != 4:
...
def input_processing(model_path: str, net_inputs: dict, input_file: str, layers_map: dict = None):
    ...
    if input_file is None:
        for net_input in net_inputs:
            inputs[net_input] = np.clip(np.random.normal(0.5, 0.1, size=net_inputs[net_input]), 0, 1)
        dump_output_file(model_path + '_random_input_dump.npz', inputs)
        return inputs
    try:
